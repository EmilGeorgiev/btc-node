...
func TestMsgTxUnmarshalBinary(t *testing.T) {
	tests := []struct {
		...
		{name: "legacy",
			...
			expected: &p2p.MsgTx{
				...
				TxWitness: p2p.TxWitnessData{Count: 0, Witness: nil},
				LockTime:  0},
			err: nil},
		{name: "segwit",
			...
			expected: &p2p.MsgTx{
				...
				TxWitness: p2p.TxWitnessData{
					Count: 0x04,
					Witness: []p2p.TxWitness{
						{},
						{
							Length: 0x47,
							Data: []uint8{
	...
	for _, test := range tests {
		t.Run(test.name, func(tt *testing.T) {
			...
			if !reflect.DeepEqual(test.expected, &tx) {
				tt.Errorf("expected :%+v\n", test.expected)
				tt.Errorf("actual   :%+v\n", &tx)
				return
			}
		})
